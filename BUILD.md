...
`make build` 默认使用当前 macOS 或 Linux/WSL 环境中的 Pandoc、XeLaTeX
和 Calibre，不会自动切换到 Docker。

在 macOS 首次构建时，如果缺少工具，脚本会通过 Homebrew 自动安装：

- Pandoc 与 Poppler；
- BasicTeX，并通过 `tlmgr` 只补齐本书使用的中文、版式和字体宏包；
- Calibre；
- Pillow，安装到 `.build/python-build-venv/` 隔离环境，不修改 Homebrew
  或系统 Python。

Homebrew cask 和 TeX Live 包安装期间，`sudo` 可能要求输入 macOS 密码。
安装完成后脚本会立即刷新 TeX/Calibre 命令路径、重新检查依赖并继续构建，
不需要手动重开终端。macOS 需要预先存在 Homebrew；如果 Homebrew 本身缺失，
脚本会给出唯一的一次性安装入口，避免静默执行远程 bootstrap 脚本。
Pillow 隔离环境随 `.build/` 一起由 `make clean` 清理，下次构建会自动重建。
脚本会在补充 TeX 宏包前先更新 `tlmgr` 和已安装的基础包，避免新安装的
BasicTeX 与当天 TeX Live 仓库版本不一致。
Calibre 主下载源中断时，脚本会从其官方 GitHub Release 重新下载，并用
Homebrew cask 声明的 SHA-256 校验后再交给 Homebrew 安装。
...
EPUB 构建通过 `scripts/epub.lua` 为代码块和重点文字添加 Apple Books 深色主题
专用容器类，`styles/epub.css` 分别提供浅色与深色配色。对不支持
`prefers-color-scheme`、还会自行改写固定颜色的旧 MOBI 阅读器，重点文字默认继承
阅读器正文色，并以粗体和金色侧边标记区分。`styles/mobi.css` 在 Calibre 转换末尾
覆盖被拍扁的 EPUB 主题颜色，避免夜间主题修好后反过来破坏浅色主题。章节标题和目录
链接只在明确的浅色主题使用品牌青色，其他情况继承阅读器前景色。验收时需要同时检查
EPUB、MOBI 的原始主题和夜间主题；重新构建后应删除阅读器中缓存的旧副本再导入。
Pandoc 的中文界面术语由 `scripts/pandoc-data/translations/zh.yaml` 提供；
构建器会按 Pandoc 能力选择新式 `--syntax-highlighting`，并为旧版本保留兼容参数。
...
未传 `VERSION` 时，脚本读取 GitHub 上最新的非草稿 Release，并立即递增
补丁版本，例如 `v0.1.9` 变为 `v0.1.10`；仓库还没有 Release 时从
`v0.1.0` 开始。`make release` 是明确的发布命令，不会再根据提交数或
发布时间静默跳过。
...
`make build-local` 是严格模式：只使用已经存在的工具，不自动安装任何系统
依赖。日常构建使用 `make build`；CI 或需要完全可复现的隔离环境使用
`make build-docker`。
